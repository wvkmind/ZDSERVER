...
Net::Connector.registerlogic('pick',-> params,my_node do
    begin
        ...
        Room.send_data(params[:user_id],{status:0,items:map.get_items,user_id: params[:user_id],pick_pos:params['pos'].to_i,id:item.get_id},params) 
    rescue Exception => e
        Log.re(e)
        my_node.send({status: 1,error:e.message},params)
    end
...
Net::Connector.registerlogic('eat',-> params,my_node do
    begin
        if Room.eat(params[:user_id],params[:pos].to_i)
            Room.send_data(params[:user_id],{status:0,eat_pos:params[:pos].to_i,items:map.get_items,user_id: params[:user_id]},params)
        else
